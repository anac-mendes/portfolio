...
const ParticleBackground = (props) => {
  ...
  const [settingsWithDefaults, setSettingsWithDefaults] = useState(
  ...
  useEffect(() => {
    if (isFirstRender.current) {
      ...
      boundCheckSettings();
      let particleChange =
        settingsWithDefaults.particle.particleCount - particles.length;
  ...
  const boundCheckSettings = () => {
    if (
      ...
      settingsWithDefaults.opacity.maxOpacity < 0
    )
      settingsWithDefaults.opacity.maxOpacity = 1;
    if (
      settingsWithDefaults.opacity.minOpacity < 0 ||
      settingsWithDefaults.opacity.minOpacity > 1
    )
      settingsWithDefaults.opacity.minOpacity = 0;
  ...
  const addParticles = (numberToAdd) => {
    ...
    for (let i = 0; i < numberToAdd; i++) {
      newParticles.push(
        new Particle(i + currentParticles.length, settingsWithDefaults, canvas)
      );
    }
  ...
  const drawBackground = () => {
    let canvas = canvasRef.current;
    if (settingsWithDefaults.canvas.canvasFillSpace && canvas && canvas.style) {
  ...
  const drawParticles = () => {
    ...
    for (let i in particles) {
      ...
      if (settingsWithDefaults.canvas.useBouncyWalls) {
        ...
        if (
          (p.x - p.size < 0 && p.vx < 0) ||
          (p.x + p.size > ctx.canvas.width && p.vx > 0)
        )
          p.x = p.getRandomInRange(p.size, ctx.canvas.width - p.size);
        if (
          (p.y - p.size < 0 && p.vy < 0) ||
          (p.y + p.size > ctx.canvas.height && p.vy > 0)
        )
      ...
      if (
        opacity.hasOwnProperty("minOpacity") &&
        opacity.hasOwnProperty("maxOpacity")
      ) {
      ...
      // Draw
      ctx.beginPath();
      ctx.fillStyle = settingsWithDefaults.particle.colors[p.color - 1];
...
class Particle {
  constructor(particleIndex, settings, canvas) {
    ...
    // Speed
    if (
      velocity.hasOwnProperty("minSpeed") &&
      velocity.hasOwnProperty("maxSpeed")
    ) {
      let speedAdjust = this.getRandomInRange(
        velocity.minSpeed,
        velocity.maxSpeed
      );
      this.vx *= speedAdjust;
    ...
    if (
      particle.hasOwnProperty("minSize") &&
      particle.hasOwnProperty("maxSize")
    ) {
    ...
    if (
      ...
      this.opacity = this.getRandomInRange(
        opacity.minOpacity,
        opacity.maxOpacity
      );
    ...
    // Color
    this.color =
      Math.floor(Math.random() * (settings.particle.colors.length - 1 + 1)) + 1;
  ...
  getSin(angle) {
    return (
      Math.round(Math.sin(this.toRadians(angle)) * 10000 + Number.EPSILON) /
      10000
    );
  }
  ...
  getCos(angle) {
    return (
      Math.round(Math.cos(this.toRadians(angle)) * 10000 + Number.EPSILON) /
      10000
    );
  }
